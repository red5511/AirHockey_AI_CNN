...
def play_episode(agent1, agent2, buffer1, buffer2, env, eps, gamma, BUFFER_BATCH, episode_number):
    ...
    while not done:
        ...
        action1 = agent1.make_move(state_4frames, eps)
        action2 = agent2.make_move(state_4frames, eps)
       ...
       # print('shape', new_state_4frames.shape, new_state.shape)
        ...
        buffer1.append(state_4frames, new_state_4frames, reward1, action1, done)
        buffer2.append(state_4frames, new_state_4frames, reward2, action2, done)
...
EPISODES_NUMBER = 15_000
BUFFER_SIZE = 135_000 #300k bierze okolo 4GB ramu dal jednego agenta
BUFFER_BATCH = 32
gamma = 0.99
learingRate = 0.001
eps_min = 0.01
eps_decay = 0.013
RESUME_TRAINING = 4080 #Przy pierwszym trenowaniu skonczylem na 1370, 2380 (2 wznowienie)
...
pritn('shape', rewards.shape[0])
if rewards.shape[0] == 5000:
    print('EXTENDING ITERS  REWARDS')
    rewards = np.pad(rewards, (0, 10_000), mode='constant', constant_values=0)
    iters = np.pad(rewards, (0, 10_000), mode='constant', constant_values=0)


buffer1 = ReplayBuffer(BUFFER_SIZE, BUFFER_BATCH, True, X, Y, "1_" + str(RESUME_TRAINING), np.sum(iters))
buffer2 = ReplayBuffer(BUFFER_SIZE, BUFFER_BATCH, True, X, Y, "2_" + str(RESUME_TRAINING), np.sum(iters))
