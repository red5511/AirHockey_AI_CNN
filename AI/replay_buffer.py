...
class ReplayBuffer():
    def __init__(self, size, BUFFER_BATCH, resume_training, SHAPE1=60, SHAPE2=30, N=0, ITERS=0, FRAMES_PER_STATE=4):
        if resume_training:
            if N == 0 or ITERS == 0:
            ...
            self.states = np.zeros((size, SHAPE2, SHAPE1, FRAMES_PER_STATE), dtype=np.int8)
            self.new_states = np.zeros((size, SHAPE2, SHAPE1, FRAMES_PER_STATE), dtype=np.int8)
            self.ready_to_train = False
            self.full_buffer = False
            self.counter = 0
            self.BUFFER_BATCH = BUFFER_BATCH
