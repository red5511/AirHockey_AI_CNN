...
def bulid_model(SHAPE, lr, env):
    ...
    model.add(layers.Conv2D(filters=16, kernel_size=8, strides=2, activation="relu", input_shape=SHAPE))
...
class Agent():
    def __init__(self, env, buffer, lr, gamma, resume_training, N=0, SHAPE=(30, 60, 4)):
        if resume_training:
            if N == 0:
