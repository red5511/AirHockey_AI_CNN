...
menu = ["1 vs 1", "1 vs AI", "Random vs Random", "Exit"]
...
while key != ESC:
	...
	if key == curses.KEY_UP:
		...
		if current_row == len(menu) - 1:
			...
			f = open('AI/renders/player_4080.json',) #3010, 3210 calkiem niezle
			buf1 = json.load(f)
			print(type(buf1), buf1)
			f2 = open('AI/renders/player2_4080.json',)#3310 porazka
			buf2 = json.load(f2)
			Air_hockey(win, X, Y, buf1, buf2)
			win.clear()
			win.border(0)
		elif current_row == 2:
			random_game = Random_game()
